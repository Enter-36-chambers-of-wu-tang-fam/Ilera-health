...
module.exports = {
  ...
  add_health_log: (params, cb) => {
    ...
    const queryString = 'INSERT INTO health_log\
      (physician_date, physician_notes, \
      physician_photo_path, id_physician, id_patient) \
      value (?, ?, ?, ?, ?)';
    db.query(queryString, data, (error, results) => cb(error, results) );
  },
  ...
  patient_update_health_log: (params, cb) => {
    ...
    const queryString = 'UPDATE health_log \
    SET patient_date=?, patient_note=?, patient_photo_path=? \
  ...
  physician_update_health_log: (params, cb) => {
    ...
    const queryString = 'UPDATE health_log \
      SET physician_date=?, physician_notes=?, physician_photo_path=? \
      WHERE id ="'+params.id+'" \
      LIMIT 1';
    db.query(queryString, data, (error, results) => cb(error, results) );
  },
  ...
  deleteOne_health_log: (params, cb) => {
    let data = [params.id];
    const queryString = 'DELETE FROM health_log \
      WHERE id=? \
      LIMIT 1';
    db.query(queryString, data, (error, results) => cb(error, results) );
  }
